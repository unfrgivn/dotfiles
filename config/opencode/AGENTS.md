...
**Purpose**: Operate Opencode CLI tasks in this repo while honoring user preferences and house style.

**When Opencode reads this**: On task initialization and before major decisions; re-skim when requirements shift.
...
- Use modules for any repeated infrastructure patterns.
- Use variables for any values that might change between environments or over time.
...
- **Homebrew default**, prefer Homebrew for installing CLIs on macOS or Linux unless the user specifies otherwise.
